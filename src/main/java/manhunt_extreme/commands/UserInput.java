...
public class UserInput implements CommandExecutor {
    ...
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        ...
        switch (label) {
            ...
            case "chestgenerate" -> {
                return new ChestGenerateCommand(manhuntPlayer, args, gameEngine, label).execute();
            }
            case "setheadstart" -> {
                return new SetHeadStartCommand(manhuntPlayer, args, gameEngine).execute();
            }
            case "runnerhelp" -> {
                return new RunnerHelpCommand(manhuntPlayer, args, gameEngine, label).execute();
            }
            case "hunterhelp" -> {
                return new HunterHelpCommand(manhuntPlayer, args, gameEngine, label).execute();
            }
            case "extradrops" -> {
                return new ExtraDropsCommand(manhuntPlayer, args, gameEngine, label).execute();
            }
            case "hasteboost" -> {
                return new HasteBoostCommand(manhuntPlayer, args, gameEngine, label).execute();
            }
            case "allhelp" -> {
                return new AllHelpCommand(manhuntPlayer, args, gameEngine, label).execute();
            }
            case "cutclean" -> {
                return new CutCleanCommand(manhuntPlayer, args, gameEngine, label).execute();
            }
            case "pause" -> {
                return new PauseCommand(manhuntPlayer, args, gameEngine, label).execute();
            }
            case "unpause" -> {
                return new UnpauseCommand(manhuntPlayer, args, gameEngine, label).execute();
            }
            case "health" -> {
                return new HealthCommand(manhuntPlayer, args, gameEngine).execute();
            }
            case "supplydrops" -> {
                return new SupplyDropsCommand(manhuntPlayer, args, gameEngine, label).execute();
            }
            case "clearteams" -> {
